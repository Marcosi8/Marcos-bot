	...
	client.on('chat-update', async (mek) => {
		try {
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*Nome do bot*: ${me.name}\n*Número do bot*: @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Bloqueios no total*: ${blocked.length}\n*O bot está ativo em*: ${kyun(uptime)}`
				...
				case 'sticker':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie fotos ou vídeos com a legenda ${prefix}sticker ou marque imagens ja enviadas!`)
					}
					break
				case 'gtts':
					if (args.length < 1) return client.sendMessage(from, '_Qual o código de idioma, cara?_', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, '?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://imgur.com/gallery/8EuqSuh`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Marcos!\n*YOUTUBE:* não tenho.\n*CONTATO:* wa.me/+558897165919\n*INSTA:* https://www.instagram.com/marcoskz_\n\n\n👋🏻✨'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'nsfwloli':
					...
					loli.getNSFWLoli(async (err, res) => {
						if (err) return reply('❌ *ERROR* ❌')
						buffer = await getBuffer(res.url)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Eu não assumo esse b.o 😳'})
					})
					break
				case 'hili':
					if (args.length < 1) return reply(', hein?')
				...
				case 'tstiker':
				case 'tsticker':
					if (args.length < 1) return reply(', hum?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'mencionar':
					...
					members_id = []
					teks = (args.length > 1) ? body.slice(8).trim() : 'adm ta chamando…'
					teks += '\n\n'
				...
				case 'limpar':
					...
					reply('Todos os chats excluídos com sucesso :)')
					break
				case 'bc':
					if (!isOwner) return reply('Quem é você? Este comando é apenas para o dono.')
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ ISSO É UMA TRANSMISSÃO ]\n\n${body.slice(4)}`})
						}
						reply('Transmissão enviada com sucesso!')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ ISSO É UMA TRANSMISSÃO ]\n\n${body.slice(4)}`)
						}
						reply('Transmissão enviada com sucesso!')
					}
					break
        case 'promover':
					...
					if (mentioned.length > 1) {
						...
						mentions(`_Você agora é um adm_ @${mentioned[0].split('@')[0]} _Divirta-se_`, mentioned, true)
				...
				case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						mentions(`_Rebaixando o adm:_ @${mentioned[0].split('@')[0]} _F no chat!_`, mentioned, true)
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('Use o código do país sem o +, amigo')
					try {
				...
				case 'bemvindo':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Recurso de boas-vindas ativado com sucesso neste grupo! ✔️')
					} else if (Number(args[0]) === 0) {
